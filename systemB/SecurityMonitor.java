...
class SecurityMonitor extends Thread
{
	...
	public void run()
	{
		...
		if (em != null)
		{
			...
			smokeIndicator = new Indicator ("Smoke Indicator", mw.GetX()+ mw.Width(), (int)(mw.Height()/3*3.5), 0);
	...
	public void setSprinkler(int state) {
		...
		if (state == 0) {
			...
			msg = new Message( Configuration.SPRINKLER_CONTROLLER_ID, "S0" );
			
		}
		else if (state==1) {
			
			msg = new Message( Configuration.SPRINKLER_CONTROLLER_ID, "S1" );
		}
	...
	public void setSecuritySystem( boolean ON )
	{
		...
		if ( ON )
		{
			...
			smokeIndicator.SetLampColorAndMessage("Smoke Safe", 1);
		} else {
			...
			smokeIndicator.SetLampColorAndMessage("Smoke Off", 0);
		} // if
